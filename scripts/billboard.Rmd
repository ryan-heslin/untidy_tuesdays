---
title: "Tidy Tuesday: Billboard Hot 100"
author: "Ryan Heslin"
...
Not suprisingly, Christmas songs come back again...and again..and again.
```{r}
billboard |>
  group_by(song) |>
...
billboard |>
  ...
  ggplot(aes(x = week_id, y = 101 - week_position, color = performer, fill = performer)) +
  geom_area(alpha = .5) +
  scale_y_continuous(breaks = c(100, 75, 50, 25, 1), labels = c(1, 25, 50, 25, 100)) +
  facet_wrap(~performer, ncol = 2, scales = "free_x") +
  labs(title = "Artists by Peak Week Chart Position", y = "Highest Week Position") +
...
combined |>
  ...
  pivot_longer(danceability:last_col(), names_to = "Feature", values_to = "Value") |>
...
longest_streak <- function(subscript, which = c("first", "last", "all")) {
  ...
  unlist(with(rle(subscript), mapply(rep, values & seq_along(lengths) %in% comparator(which(lengths == max(lengths[values]))), lengths, SIMPLIFY = FALSE)))
}
```
...
streaks <- billboard |>
  ...
  ungroup() |>
  filter(performer %in% names(sort(table(performer), decreasing = TRUE)[1:10])) |>
